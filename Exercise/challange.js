...
class buku extends Product{
    constructor(nama,berat,price,stok,author){
...
class pakaian extends Product{
    constructor(nama,berat,price,stok,size,material){
        super(nama,berat,price,stok)
        this.size = size
...
class OnlineShop {
    ...
    tambahproduk(produk){
        if(this.#products.includes(i => i.nama)){
            return 'product sudah ready dalam toko'
        } else {
    ...
    tambahKeKeranjang(produk, jumlah){
        const produkname = this.#products.find(i => i.nama === produk);
        if(!produkname)
            return 'Produk tidak ditemukan di katalog'
        if(produkname.stok < jumlah)
            return `stok tidak mencukupi. Stok saat ini ${produkname.stok}`;

        const itemIncart = this.#carts.find(item => item.produk.nama === produk);
        if(itemIncart){
            itemIncart.jumlah = itemIncart + jumlah;
            produkname.stok -= jumlah
        }else{
            this.#carts.push({produk : produkname,jumlah: jumlah});
            produkname.stok -= jumlah;
        }
    }

    transaksi(uangUser, jarak){
        const totalharga = this.#carts.reduce((total,item)=> total + (item.produk.price * item.jumlah),0);
        const ongkir = jarak * 2000;
        const totalpembayaran = totalharga + ongkir;

        if(uangUser < totalpembayaran){
            return `uang tidak cukup. Total pembayaran: ${totalpembayaran}`;
        }
    ...
    showCatalog(){
        return this.#products.map(i => `${i.nama} - Stok: ${i.stok}`).join('\n');
    }

    showCart(){
        return this.#carts.map(item => `${item.produk.nama} - Jumlah: ${item.jumlah}`).join('\n');
    }
}

let onlineShop = new OnlineShop();
onlineShop.tambahproduk(new buku('Laskar Pelangi',10,0,50))
